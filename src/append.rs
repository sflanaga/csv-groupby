...
fn main() -> Result<(), Box<dyn Error>> {
    let mut cli: AppendCli = AppendCli::from_args();
    let start_f = Instant::now();

    let stdout = std::io::stdout();
    let mut writerlock = stdout.lock();

    let (mut reader, mut writer) = get_reader_writer();
    reader.for_byte_line(|line| {
        if line.len() <= 0 { () }
        if cli.prefix_str.len() > 0 { writer.write(cli.prefix_str.as_bytes())?; }
        writer.write(line.as_bytes())?;
        if cli.append_str.len() > 0 { writer.write(cli.append_str.as_bytes())?; }
        writer.write(&[b'\n' as u8])?;
        Ok(true)
    })?;
    writer.flush()?;
    let end_f = Instant::now();
    if cli.verbose > 0 { eprintln!("runtime: {} secs", (end_f-start_f).as_secs()); }
    Ok(())
}